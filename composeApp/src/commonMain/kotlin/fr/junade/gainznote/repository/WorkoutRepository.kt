package fr.junade.gainznote.repository

import fr.junade.gainznote.db.DatabaseDriverFactory
import fr.junade.gainznote.db.GainzNoteDatabase
...
import fr.junade.gainznote.model.CircuitConfig
...
class WorkoutRepository(driverFactory: DatabaseDriverFactory) {
    ...
    private fun buildWorkout(id: String): Workout {
        ...
        val exercises = if (type == WorkoutType.MUSCULATION) {
            q.getExercisesForWorkout(id).executeAsList().map { ex ->
                ...
                Exercise(ex.id, ex.workout_id, ex.name, ex.position.toInt(), ex.superset_with, ex.superset_group?.toInt(), sets)
            }
        } else emptyList()
        ...
        db.transaction {
            ...
            if (workout.type == WorkoutType.MUSCULATION) {
                workout.exercises.forEachIndexed { i, ex ->
                    q.insertExercise(ex.id, workout.id, ex.name, i.toLong(), ex.supersetWith, ex.supersetGroup?.toLong())
    ...
    suspend fun hasWorkouts(): Boolean = withContext(Dispatchers.IO) {
        q.countWorkouts().executeAsOne() > 0
    }
            ...
            workouts.forEachIndexed { wi, w ->
                ...
                if (w.type == WorkoutType.MUSCULATION) {
                    ...
                    w.exercises.forEachIndexed { ei, ex ->
                        ...
                        append("\"supersetWith\":${ex.supersetWith.jn()},\"supersetGroup\":${ex.supersetGroup ?: "null"},")
                        append("\"sets\":[")
    ...
    private fun sf(json: String, key: String): String? =
        regexCache.getOrPut("sf_$key") { "\"$key\"\\s*:\\s*\"((?:[^\"\\\\]|\\\\.)*)\"".toRegex() }
            .find(json)?.groupValues?.getOrNull(1)?.replace("\\\"","\"")?.replace("\\\\","\\")?.replace("\\n","\n")

    private fun snf(json: String, key: String): String? {
        if (regexCache.getOrPut("null_$key") { "\"$key\"\\s*:\\s*null".toRegex() }.containsMatchIn(json)) return null
        return sf(json, key)
    }

    private fun df(json: String, key: String): Double? {
        if (regexCache.getOrPut("null_$key") { "\"$key\"\\s*:\\s*null".toRegex() }.containsMatchIn(json)) return null
        return regexCache.getOrPut("df_$key") { "\"$key\"\\s*:\\s*(-?[0-9.eE+\\-]+)".toRegex() }.find(json)?.groupValues?.getOrNull(1)?.toDoubleOrNull()
    }

    private fun inf(json: String, key: String): Int? {
        if (regexCache.getOrPut("null_$key") { "\"$key\"\\s*:\\s*null".toRegex() }.containsMatchIn(json)) return null
        return regexCache.getOrPut("int_$key") { "\"$key\"\\s*:\\s*(-?[0-9]+)".toRegex() }.find(json)?.groupValues?.getOrNull(1)?.toIntOrNull()
    }

    private fun lnf(json: String, key: String): Long? {
        if (regexCache.getOrPut("null_$key") { "\"$key\"\\s*:\\s*null".toRegex() }.containsMatchIn(json)) return null
        return regexCache.getOrPut("lng_$key") { "\"$key\"\\s*:\\s*(-?[0-9]+)".toRegex() }.find(json)?.groupValues?.getOrNull(1)?.toLongOrNull()
    }
    ...
    private fun parseExerciseJson(json: String, workoutId: String): Exercise {
        ...
        return Exercise(exId, workoutId, sf(json, "name") ?: "",
            inf(json, "position") ?: 0, snf(json, "supersetWith"), inf(json, "supersetGroup"), sets)
    }
