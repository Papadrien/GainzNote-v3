...
class WorkoutViewModel(
    ...
    fun linkSuperset(aId: String, bId: String) = updateExercises {
        ...
        val group = current.firstOrNull { it.id == aId }?.supersetGroup
            ?: current.firstOrNull { it.id == bId }?.supersetGroup
            ?: ((current.mapNotNull { it.supersetGroup }.maxOrNull() ?: 0) + 1)
        map {
            if (it.id == aId || it.id == bId) it.copy(supersetGroup = group)
            else it
        }
    }

    fun unlinkSuperset(exId: String) {
        updateExercises { map { if (it.id == exId) it.copy(supersetGroup = null, supersetWith = null) else it } }
    }
