        ...
        Column(Modifier.fillMaxSize().verticalScroll(rememberScrollState()).padding(16.dp)) {
            ...
            workout.exercises.forEachIndexed { idx, ex ->
                ExerciseCard(
                    ...
                    isSupersetMember = ex.supersetGroup != null,
...
fun ExerciseCard(
    ...
    Column(Modifier.fillMaxWidth()
        .border(if (isSupersetMember) 2.dp else 1.dp,
            if (isSupersetMember) supersetColor(exercise.supersetGroup, c) else c.border, RoundedCornerShape(14.dp))
        .background(c.surface, RoundedCornerShape(14.dp))) {

        Row(Modifier.padding(start = 12.dp, end = 4.dp, top = 10.dp, bottom = 4.dp),
            verticalAlignment = Alignment.CenterVertically) {
            if (isSupersetMember) {
                Box(Modifier.background(supersetColor(exercise.supersetGroup, c).copy(alpha = 0.15f), RoundedCornerShape(5.dp))
                    .padding(horizontal = 6.dp, vertical = 2.dp)) {
                    Text("SS${exercise.supersetGroup ?: ""}", color = supersetColor(exercise.supersetGroup, c), fontSize = 9.sp, fontWeight = FontWeight.Bold)
                }
        ...
        HorizontalDivider(color = if (isSupersetMember) supersetColor(exercise.supersetGroup, c).copy(alpha = 0.3f) else c.border, thickness = 0.5.dp)
...
private fun supersetColor(group: Int?, c: GainzThemeColors): Color = when (((group ?: 1)-1)%3) {
1 -> Color(0xFFFF9800)
2 -> Color(0xFF8BC34A)
else -> c.superset
}
