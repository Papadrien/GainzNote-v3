...
import fr.junade.gainznote.ui.home.formatDisplayDateFull
...
fun WorkoutScreen(
    ...
    BoxWithConstraints(Modifier.fillMaxSize().background(c.background)) {
        ...
        Column(Modifier.fillMaxSize().verticalScroll(rememberScrollState()).padding(16.dp)) {
            ...
            // Calculer l'index de chaque groupe superset (SS1, SS2, SS3...)
            val supersetOrder = mutableListOf<String>()
            workout.exercises.forEach { ex ->
                val sid = ex.supersetId
                if (sid != null && sid !in supersetOrder) supersetOrder.add(sid)
            }
    ...
    // Dialogue superset multi-sélection
    showSupersetPicker?.let { srcId ->
        val srcExercise = workout.exercises.firstOrNull { it.id == srcId }
        val existingGroupId = srcExercise?.supersetId
        // Candidats : tous les exercices sauf la source
        // Si la source est déjà dans un groupe, on peut aussi ajouter des exercices hors groupe
        val candidates = workout.exercises.filter { it.id != srcId }
        var selectedIds by remember(srcId) {
            mutableStateOf(
                if (existingGroupId != null)
                    candidates.filter { it.supersetId == existingGroupId }.map { it.id }.toSet()
                else emptySet()
            )
        }
        AlertDialog(
            ...
            text = {
                if (candidates.isEmpty()) Text(S.noExerciseAvailable, color = c.textSec)
                    ...
                    Text(
                        "Sélectionnez les exercices à associer :",
                        color = c.textSec, fontSize = 13.sp,
                        modifier = androidx.compose.ui.Modifier.padding(bottom = 8.dp)
                    )
                    candidates.forEach { ex ->
                        val checked = ex.id in selectedIds
                        Row(
                            verticalAlignment = Alignment.CenterVertically,
                            modifier = Modifier.fillMaxWidth()
                                .clickable {
                                    selectedIds = if (checked) selectedIds - ex.id else selectedIds + ex.id
                                }
                            ...
                            Checkbox(
                                checked = checked,
                                onCheckedChange = { v ->
                                    selectedIds = if (v) selectedIds + ex.id else selectedIds - ex.id
                                },
                                colors = CheckboxDefaults.colors(checkedColor = c.superset)
                            )
                            Spacer(Modifier.width(8.dp))
                            Text(ex.name.ifBlank { S.unnamedExercise }, color = c.text, fontSize = 15.sp)
                        }
                    }
                }
            },
            confirmButton = {
                Button(
                    onClick = {
                        if (selectedIds.isNotEmpty()) vm.linkSuperset(srcId, selectedIds.toList())
                        showSupersetPicker = null
                    },
                    enabled = selectedIds.isNotEmpty(),
                    colors = ButtonDefaults.buttonColors(containerColor = c.superset)
...
fun ExerciseCard(
    ...
    val ssColorDim = if (isSupersetMember) supersetColorDim(supersetIndex) else c.supersetDim
    val ssLabel = if (isSupersetMember) "SS$supersetIndex" else "SS"

    Column(Modifier.fillMaxWidth()
        .border(if (isSupersetMember) 2.dp else 1.dp,
            if (isSupersetMember) ssColor else c.border, RoundedCornerShape(14.dp))
        ...
        Row(Modifier.padding(start = 12.dp, end = 4.dp, top = 10.dp, bottom = 4.dp),
            ...
            if (isSupersetMember) {
                Box(Modifier.background(ssColorDim, RoundedCornerShape(5.dp))
                    ...
                    Text(ssLabel, color = ssColor, fontSize = 9.sp, fontWeight = FontWeight.Bold)
                }
                Spacer(Modifier.width(8.dp))
            }
            ...
            Box {
                ...
                DropdownMenu(expanded = menuOpen, onDismissRequest = { menuOpen = false }, containerColor = c.surface) {
                    ...
                    if (isSupersetMember)
                        DropdownMenuItem(text = { Text(S.unlinkSuperset, color = ssColor) },
        ...
        HorizontalDivider(color = if (isSupersetMember) ssColor.copy(alpha = 0.3f) else c.border, thickness = 0.5.dp)
